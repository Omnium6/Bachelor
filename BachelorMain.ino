...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//Sleepmode ting
rtos::EventFlags event_flags; // Definer et EventFlags objekt
const uint32_t D3_INTERRUPT_FLAG = (1UL << 0); // Definer et flag (bit 0)
...
BLECharacteristic* pTimeSetCharacteristic; // NY: Til at modtage tid
const char* TIME_SET_CHAR_UUID = "6e400002-b5a3-f393-e0a9-e50e24dcca9e"; // Din valgte UUID
...
void setup() {
  ...
  fallConfirmed= false;
  Serial.begin(115200);
  //%%%%%%%%%%%%%%%%%%%%%MEGET FUCKING VIGTIGT ELLERS VIRKER DEN IKKE VEL!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
  while (!Serial); // Husk at fucking fjerne den her når den endelige kode er din dummem dumme stodder
  //%%%%%%%%%%%%%%%%%%%%%MEGET FUCKING VIGTIGT ELLERS VIRKER DEN IKKE VEL!!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
  setupEPD();
  setupBLE();
  setupFall();
