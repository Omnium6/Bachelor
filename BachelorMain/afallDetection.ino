...
void checkIfFallOccured(){ //Tilføj så den chekker om hændelse er fald og sæt flag tilsvarende
  ...
  while (samplesRead < numSamples) {
    Serial.println("Model begyndt");
    
    if (!IMU.accelerationAvailable()) {
      Serial.println("Nul imu acc");
    }
    if (!IMU.gyroscopeAvailable()) {
      Serial.println("Nul imu gyrp");
    }
    
    // check if a sample is available
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
      // read acceleration and gyroscope values
      Serial.println("IMU i gang");
      ...
      Serial.println("Læser samples eller noget");
      
      // if all samples are got, run inference
      if (samplesRead == numSamples) {
        Serial.println("Kører model");
        ...
        Serial.println();
        if (modelGetOutput(0) > 0.5) { //Måske er fald en anden gesture skal lige kigges på
                                       // samt skal 0.5 måske også ændres med det er emd test
          fallConfirmed = true;
          Serial.println("Fall confirmed by model");
        }
        else {
          fallSuspected = false;
          Serial.println("Fall not confirmed by model");
        }
      }
    }
  }
...
void checkIfFallOccured() {
  ...
  // Nulstil sample-tælleren
  samplesRead = 0;
  
  float aX, aY, aZ, gX, gY, gZ;

  // Løkken kører, indtil det nødvendige antal samples er indsamlet.
  // Denne struktur er identisk med den i det velfungerende eksempel.
  while (samplesRead < numSamples) {
    // Tjek om en ny måling er tilgængelig.
    if(!IMU.accelerationAvailable()) {
      //Serial.println("Du er en bæ!!");
    }
    if (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
    ...
    // Intet delay er nødvendigt, hvis IMU'en sampler korrekt.
  }
...
void checkIfFallOccured() {
  ...
  if (modelGetOutput(0) > 0.5) { // Fald bekræftet
    fallConfirmed = true;
    fallSuspected = false;
    Serial.println("Fald bekræftet af model.");
  } else {
